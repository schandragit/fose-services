...
However, given the growth in testing services, it is inevitable that it needs to
move from a labor intensive business to an assets and tools-based business.
Therefore, research in this area is about automated tools so that testing can be
carried out at lower cost and with consistent quality.  Implicit in the
abovementioned reasons is that the proposed solutions should be quick to deploy,
low cost, and accessible to a large labor pool.
...
rendering.
%%, say across browsers or application variants.
...
failure observed on a new application version can either expose application
faults or result from a problem with the test itself. Determining this reason
for failure is the critical first step before any corrective action can be
taken---repairing the test if the test is broken, or fixing the application if
the test has revealed an application fault. Without tool assistance and faced
with a large number of test failures, testers can tend to ignore test-execution
results or, even worse, delete failing tests, which can quickly degrade test
coverage and defeat the purpose of testing.
